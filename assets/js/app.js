    ...
    <article class="catalog-card reveal">
      ...
      <div class="catalog-body">
        ...
        <div class="card-actions">
          <a class="btn btn-solid" href="#reservas">Reservar cita</a>
...
function validateBirthDate(value) {
  ...
  if (age < 18) return 'El lead debe ser mayor de edad.';
  return '';
}
...
leadForm?.addEventListener('submit', async (event) => {
  ...
  if (error) {
    setFeedback(leadFeedback, error, 'error');
    return;
  }

  const payload = {
    fullName: String(formData.get('leadName') || '').trim(),
    birthDate,
    email: String(formData.get('leadEmail') || '').trim(),
    phone: String(formData.get('leadPhone') || '').trim(),
  ...
  try {
    await persistToFirebase('leads', payload);
    leadForm.reset();
    setFeedback(leadFeedback, firebaseConfig
      ? 'Lead guardado correctamente en Firebase/Firestore.'
      : 'Lead validado. Para guardarlo en la nube, agrega tu configuracion Firebase en assets/js/firebase-config.example.js.', 'success');
  } catch (error) {
    setFeedback(leadFeedback, 'No se pudo guardar el lead. Revisa la configuracion de Firebase.', 'error');
    console.error(error);
  }
});
...
bookingForm?.addEventListener('submit', async (event) => {
  ...
  if (!appointmentDate || Number.isNaN(selected.getTime()) || selected < today) {
    setFeedback(bookingFeedback, 'La fecha de cita debe ser hoy o una fecha futura.', 'error');
    return;
  }
  ...
  try {
    ...
    bookingForm.reset();
    setFeedback(bookingFeedback, firebaseConfig
      ? 'Reserva guardada correctamente en Firebase/Firestore.'
      : 'Reserva validada. Para guardarla en la nube, agrega tu configuracion Firebase en assets/js/firebase-config.example.js.', 'success');
  } catch (error) {
